...
function CustomCursor() {
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      const isEditable = closestElement(event.target, editableSelector) !== null;
      setVisibleSafely(!isEditable);
      setInteractiveSafely(!isEditable && closestElement(event.target, interactiveSelector) !== null);
    };
  ...
  return (
    <>
      ...
      >
        <motion.span
          className="absolute -left-5 -top-5 h-10 w-10 rounded-full border border-white/45 bg-white/5 shadow-[0_0_30px_rgba(48,169,220,0.2)] backdrop-blur-[1px]"
          animate={{
            opacity: visible ? 1 : 0,
        ...
        />
        <motion.span
          className="absolute -left-8 -top-8 h-16 w-16 rounded-full bg-[conic-gradient(from_180deg,var(--brand-cyan),var(--brand-green),var(--brand-yellow),var(--brand-red),var(--brand-purple),var(--brand-cyan))] opacity-20 blur-xl"
          animate={{
